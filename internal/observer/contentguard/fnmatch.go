...
// NOT filepath.Match. This is a verbatim port of the recall forwarder's hardened matcher;
// the observer content lane may not import internal/recallaxis (it is a banned telemetry
// axis — see cmd/gasworks-observer/boundary_test.go), so the engine is re-implemented here
// rather than shared. The differences from filepath.Match that matter for the secrets
// deny-globs:
//
...
//     the fnmatch rule so the behavior is identical and provably so; a switch to
//     filepath.Match would silently weaken globs like "*.env".
...
//
// Implemented as a direct iterative glob matcher (the classic two-pointer backtracking
// algorithm) so there is no regex-translation surface to get subtly wrong.
