...
func TestBackfill_ReplaysArchivesAndLiveInChunks(t *testing.T) {
	...
	for _, b := range batches {
		if b.CityHash != eventexport.CityHash(cfg.Salt, "testcity") || b.SchemaVersion != eventexport.SchemaVersion {
	...
	// The projection's content gate (Options.emitContent) is unexported upstream,
	// so the lifted title/formula must NOT egress; the lift's surviving effect is
	// the run_id/step_id fallback derived from the flat payload's gc.* metadata.
	closed := byType["bead.closed"]
	if closed.Title != "" || closed.Formula != "" {
		t.Errorf("content leaked past the unexported emitContent gate: %+v", closed)
	}
	if closed.StepID != "review" || closed.RunID != "mc-wisp-root1" {
		t.Errorf("flat-payload correlation lift missing on bead.closed: %+v", closed)
	}
...
// TestExporter_DrainsBacklogInBatchMaxChunks pins the exporter's rate-limit
// invariant (now provided natively upstream via the eager BatchMax flush and
// over-cap ingest backpressure): a backlog larger than the ingest's per-request
// budget drains in BatchMax-bounded POSTs, each advancing the cursor, instead
// of retrying one oversized batch forever.
