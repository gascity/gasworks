...
func (r *Runner) Run(ctx context.Context) error {
	...
	if err != nil {
		// A missing/corrupt resume point must not silently become a zero cursor:
		// that would replay retained city history to the ingest endpoint.
		return fmt.Errorf("events: refusing to start with unreadable cursor state %q: %w", r.cfg.StatePath, err)
	}
	...
	exp := eventexport.New(eventexport.Config{
		...
		// Content is deliberately not configurable in eventexport's current API.
