...
func Backfill(ctx context.Context, cfg Config, city string, files []string, fromSeq, toSeq uint64, dryRun bool, logf Logf) (BackfillResult, error) {
	...
	// so Title/Formula lifted below can never egress from here; under
	// cfg.EmitContent the lift still supplies the run_id/step_id fallback for
	// producers that predate the envelope fields — same as the live axis.
...
func postChunk(ctx context.Context, client *http.Client, cfg Config, city string, chunk []eventexport.Envelope) (ingestResponse, error) {
	body, err := json.Marshal(eventexport.Batch{CityHash: eventexport.CityHash(cfg.Salt, city), SchemaVersion: eventexport.SchemaVersion, Events: chunk})
	if err != nil {
