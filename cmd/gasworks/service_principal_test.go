...
func TestServicePrincipalExchangePinnedToMachineFallbackOrigin(t *testing.T) {
	...
	request := `{"version":"gascity.dev/credential-provider/v1","audience":"manifold","required_scopes":["manifold:proxy"],"org":"org_a","interactive":false}`
	cfg := config.Config{STSCanonical: canonical.URL, STSBase: legacy.server.URL}
	...
	if canonicalMachine != 1 || canonicalToken != 0 {
		t.Fatalf("canonical requests machine=%d token=%d, want machine=1 token=0", canonicalMachine, canonicalToken)
	}
